...
# # PROMPT_TEMPLATE = """
# # You are an IT auditor tasked with evaluating the alignment between the provided evidence and the security controls outlined in the given policy. Your task is to assess the information in a methodical and comprehensive manner based strictly on the evidence and policy provided. Do **not** make assumptions beyond the data.

# # ### Subject of Analysis: {subject}

# # ### Instructions:
# # 1. **Detailed Observations**:
# #    - Document matches and mismatches between the policy's requirements and evidence provided.
# #    - Integrate relevant policy requirement text for context.

# # 2. **Identification of Gaps**:
# #    - Highlight missing configurations or controls.

# # 3. **Actionable Recommendations**:
# #    - Provide recommendations for each gap tied to policy requirements.
...
# PROMPT_TEMPLATE = """
# You are an IT auditor tasked with evaluating the alignment between the provided evidence and the security controls outlined in the given policy. Your analysis must be methodical, comprehensive, and strictly based on the provided data—do **not** make assumptions beyond what is given.

# ### Task Description:
# Using the provided policy and evidence, produce an audit report that:
# - Identifies and documents matches and mismatches between the policy requirements and the evidence.
# - Highlights missing configurations or controls.
# - Provides actionable, specific recommendations for addressing any identified gaps.

# ### Background & Context:
# This task is based on principles outlined in DAIR.AI (2024), which emphasize:
# - **Clarity and Specificity:** Clearly defining the task helps ensure accurate and relevant responses.
# - **Contextual Information:** Providing background enables the model to understand the task and deliver detailed analysis.
# - **Examples and Constraints:** Offering guidance on the desired output format and limitations ensures that the response meets expected criteria.

# ### Instructions:
# 1. **Detailed Observations:**
#    - Compare the evidence against the policy requirements and document both matches and mismatches.
#    - Integrate relevant excerpts from the policy text to provide context.
#    - Use bullet points or numbered lists to structure your observations clearly.

# 2. **Identification of Gaps:**
#    - Identify any missing configurations or controls.
#    - If any evidence is incomplete or ambiguous, specify what additional data would be necessary to complete the assessment.

# 3. **Actionable Recommendations:**
#    - For each identified gap, provide specific, clear, and feasible recommendations that align with the corresponding policy requirement.

# ### Formatting Requirements:
# - **Headings & Subheadings:** Use clear headings (as provided) to separate sections.
# - **Lists:** Organize observations, gaps, and recommendations using bullet points or numbered lists.
# - **Clarity & Conciseness:** Start with a brief summary, then elaborate on each point as necessary.
# - **Style & Tone:** Maintain a formal and objective tone appropriate for an IT audit report.
...
#         # Parse result into specific sections based on expected format
#         observations = ""
#         gaps = ""
#         recommendations = ""

#         # Look for key sections in the response
#         if "Detailed Observations:" in result:
#             observations = result.split("Detailed Observations:")[1].split("Identification of Gaps:")[0].strip()

#         if "Identification of Gaps:" in result:
#             gaps = result.split("Identification of Gaps:")[1].split("Actionable Recommendations:")[0].strip()

#         if "Actionable Recommendations:" in result:
#             recommendations = result.split("Actionable Recommendations:")[1].strip()

#         # Ensure each section has content, else set as "Not provided"
#         observations = observations if observations else "Not provided"
#         gaps = gaps if gaps else "Not provided"
#         recommendations = recommendations if recommendations else "Not provided"

#         return observations, gaps, recommendations
...
#             observations, gaps, recommendations = analyze_design(policy_content, evidence_content, subject)
